# Install script for kubectl on Windows
...
function Install-KubectlWithWinget {
    Write-LogInfo "Installing kubectl via winget..."
    $attempt = 0
    while ($attempt -lt $MAX_RETRY_ATTEMPTS) {
        $attempt++
        try {
            $result = winget install --id Kubernetes.kubectl --accept-source-agreements --accept-package-agreements --silent 2>&1
            if ($LASTEXITCODE -eq 0 -or $result -match "already installed") {
                Write-LogSuccess "kubectl installed successfully via winget"
                return $true
            }
        } catch {
    ...
    Write-LogError "Failed to install kubectl via winget after $MAX_RETRY_ATTEMPTS attempts"
    return $false
}

# Install kubectl using Chocolatey
function Install-KubectlWithChocolatey {
    Write-LogInfo "Installing kubectl via Chocolatey..."
    $attempt = 0
    while ($attempt -lt $MAX_RETRY_ATTEMPTS) {
        $attempt++
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-LogSuccess "kubectl installed successfully via Chocolatey"
                return $true
            }
        } catch {
    ...
    Write-LogError "Failed to install kubectl via Chocolatey after $MAX_RETRY_ATTEMPTS attempts"
    return $false
}

# Refresh PATH to find newly installed kubectl
...
function Test-KubectlInstallation {
    Write-LogInfo "Verifying kubectl installation..."
    Update-PathEnvironment

    $kubectlCommand = Get-Command kubectl -ErrorAction SilentlyContinue

    if (-not $kubectlCommand) {
        Write-LogError "kubectl command not found after installation"
        return $false
    }
    ...
    try {
        ...
        if ([string]::IsNullOrEmpty($kubectlVersion)) {
            Write-LogError "Could not retrieve kubectl version"
            return $false
        }

        Write-LogSuccess "kubectl verified: $kubectlVersion"
        return $true
    } catch {
        Write-LogError "Failed to verify kubectl: $_"
        return $false
    }
}
...
# Main entry point
function Main {
    Write-LogInfo "Starting kubectl installation on Windows..."
    ...
    if (Test-WingetInstalled) {
        ...
        $installed = Install-KubectlWithWinget
    }
    ...
    if (-not $installed) {
        ...
        $installed = Install-KubectlWithChocolatey
    }

    if (-not $installed) {
        Write-LogError "Failed to install kubectl with any available package manager"
        exit 1
    }

    if (-not (Test-KubectlInstallation)) {
