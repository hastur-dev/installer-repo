# Install script for FFmpeg on Windows
...
function Install-FFmpegWithWinget {
    Write-LogInfo "Installing FFmpeg via winget..."
    $attempt = 0
    while ($attempt -lt $MAX_RETRY_ATTEMPTS) {
        $attempt++
        try {
            $result = winget install --id Gyan.FFmpeg --accept-source-agreements --accept-package-agreements --silent 2>&1
            if ($LASTEXITCODE -eq 0 -or $result -match "already installed") {
                Write-LogSuccess "FFmpeg installed successfully via winget"
                return $true
            }
        } catch {
    ...
    Write-LogError "Failed to install FFmpeg via winget after $MAX_RETRY_ATTEMPTS attempts"
    return $false
}

# Install FFmpeg using Chocolatey
function Install-FFmpegWithChocolatey {
    Write-LogInfo "Installing FFmpeg via Chocolatey..."
    $attempt = 0
    while ($attempt -lt $MAX_RETRY_ATTEMPTS) {
        $attempt++
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-LogSuccess "FFmpeg installed successfully via Chocolatey"
                return $true
            }
        } catch {
    ...
    Write-LogError "Failed to install FFmpeg via Chocolatey after $MAX_RETRY_ATTEMPTS attempts"
    return $false
}
...
function Test-FFmpegInstallation {
    Write-LogInfo "Verifying FFmpeg installation..."
    Update-PathEnvironment

    $ffmpegCommand = Get-Command ffmpeg -ErrorAction SilentlyContinue

    if (-not $ffmpegCommand) {
        Write-LogError "ffmpeg command not found after installation"
        return $false
    }
    ...
    try {
        ...
        if ([string]::IsNullOrEmpty($ffmpegVersion)) {
            Write-LogError "Could not retrieve FFmpeg version"
            return $false
        }

        Write-LogSuccess "FFmpeg verified: $ffmpegVersion"
        return $true
    } catch {
        Write-LogError "Failed to verify FFmpeg: $_"
        return $false
    }
}
...
# Main entry point
function Main {
    Write-LogInfo "Starting FFmpeg installation on Windows..."
    ...
    if (-not $installed) {
        ...
        $installed = Install-FFmpegWithChocolatey
    }

    if (-not $installed) {
        Write-LogError "Failed to install FFmpeg with any available package manager"
        exit 1
    }

    if (-not (Test-FFmpegInstallation)) {
        Write-LogError "FFmpeg installation verification failed"
        exit 1
    }
