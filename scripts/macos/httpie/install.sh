#!/usr/bin/env bash
# Install script for HTTPie on macOS
...
install_httpie() {
    log_info "Installing HTTPie via Homebrew..."

    if brew list httpie &> /dev/null; then
        log_info "HTTPie is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade httpie 2>&1 || true; then
                log_success "HTTPie upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        if brew install httpie 2>&1; then
...
# Main entry point
main() {
    log_info "Starting HTTPie installation on macOS..."
