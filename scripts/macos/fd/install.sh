#!/usr/bin/env bash
# Install script for fd on macOS
...
install_fd() {
    ...
    if brew list fd &> /dev/null; then
        log_info "fd is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade fd 2>&1 || true; then
                log_success "fd upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
verify_installation() {
    ...
    if ! command -v fd &> /dev/null; then
        log_error "fd command not found after installation"
        return 1
    fi
...
# Main entry point
main() {
    log_info "Starting fd installation on macOS..."
