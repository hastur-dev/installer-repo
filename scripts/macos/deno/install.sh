#!/usr/bin/env bash
# Install script for Deno on macOS
# Uses Homebrew or official installer
...
install_deno() {
    log_info "Installing Deno via Homebrew..."

    if brew list deno &> /dev/null; then
        log_info "Deno is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade deno 2>&1 || true; then
                log_success "Deno upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting Deno installation on macOS..."
