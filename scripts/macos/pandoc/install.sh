#!/usr/bin/env bash
# Install script for Pandoc on macOS
...
install_pandoc() {
    log_info "Installing Pandoc via Homebrew..."

    if brew list pandoc &> /dev/null; then
        log_info "Pandoc is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade pandoc 2>&1 || true; then
                log_success "Pandoc upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        if brew install pandoc 2>&1; then
...
# Main entry point
main() {
    log_info "Starting Pandoc installation on macOS..."
