#!/usr/bin/env bash
# Install script for GitHub CLI (gh) on macOS
...
install_gh() {
    ...
    if brew list gh &> /dev/null; then
        log_info "GitHub CLI is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade gh 2>&1 || true; then
                log_success "GitHub CLI upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting GitHub CLI installation on macOS..."
