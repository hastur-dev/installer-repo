#!/usr/bin/env bash
# Uninstall script for GitHub CLI (gh) on macOS
# Removes GitHub CLI installed via Homebrew
...
# Check if GitHub CLI is installed via Homebrew
is_gh_installed_via_brew() {
...
uninstall_gh() {
    log_info "Uninstalling GitHub CLI via Homebrew..."

    local attempt=0
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ((attempt++)) || true

        if brew uninstall gh 2>&1; then
...
verify_uninstall() {
    ...
    if is_gh_installed_via_brew; then
        log_error "GitHub CLI is still installed via Homebrew"
        return 1
    fi
...
# Main entry point
main() {
    log_info "Starting GitHub CLI uninstallation on macOS..."
    ...
    if ! is_gh_installed_via_brew; then
        log_info "GitHub CLI is not installed via Homebrew, nothing to uninstall"
        exit 0
    fi
