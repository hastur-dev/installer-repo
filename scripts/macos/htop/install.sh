#!/usr/bin/env bash
# Install script for htop on macOS
...
install_htop() {
    log_info "Installing htop via Homebrew..."

    if brew list htop &> /dev/null; then
        log_info "htop is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade htop 2>&1 || true; then
                log_success "htop upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting htop installation on macOS..."
