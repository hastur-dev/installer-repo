#!/usr/bin/env bash
# Install script for Ruby on macOS
...
# Install Ruby with retry
install_ruby() {
    log_info "Installing Ruby via Homebrew..."

    if brew list ruby &> /dev/null; then
        log_info "Ruby is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade ruby 2>&1 || true; then
                log_success "Ruby upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
    ...
    log_error "Failed to install Ruby after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}
...
verify_installation() {
    ...
    if ! command -v ruby &> /dev/null; then
        log_error "ruby command not found after installation"
        return 1
    fi
    ...
    if [ -z "$ruby_version" ]; then
        log_error "Could not retrieve Ruby version"
        return 1
    fi

    log_success "Ruby verified: ${ruby_version}"

    if command -v gem &> /dev/null; then
        local gem_version
        gem_version=$(gem --version 2>&1)
        log_success "gem verified: ${gem_version}"
    fi
...
# Main entry point
main() {
    log_info "Starting Ruby installation on macOS..."
