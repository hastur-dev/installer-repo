#!/usr/bin/env bash
# Install script for tree on macOS
...
install_tree() {
    log_info "Installing tree via Homebrew..."

    if brew list tree &> /dev/null; then
        log_info "tree is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade tree 2>&1 || true; then
                log_success "tree upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting tree installation on macOS..."
