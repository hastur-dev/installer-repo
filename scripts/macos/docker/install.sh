#!/usr/bin/env bash
# Install script for Docker on macOS
# Uses Homebrew to install Docker Desktop
...
install_docker() {
    ...
    if brew list --cask docker &> /dev/null; then
        log_info "Docker Desktop is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade --cask docker 2>&1 || true; then
                log_success "Docker Desktop upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
    ...
    log_error "Failed to install Docker Desktop after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}
...
verify_installation() {
    ...
    # Docker CLI may not be available until Docker Desktop is started
    if ! command -v docker &> /dev/null; then
        log_info "docker command not found - Docker Desktop may need to be started"
        log_info "Please launch Docker Desktop from Applications"
        return 0
    fi
...
# Main entry point
main() {
    log_info "Starting Docker installation on macOS..."
    ...
    log_info "Note: You may need to start Docker Desktop from Applications"
