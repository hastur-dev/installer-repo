#!/usr/bin/env bash
# Uninstall script for Nmap on macOS
# Removes Nmap installed via Homebrew
...
# Check if Nmap is installed via Homebrew
is_nmap_installed_via_brew() {
    if brew list nmap &> /dev/null 2>&1; then
        return 0
    fi
    return 1
}
...
uninstall_nmap() {
    log_info "Uninstalling Nmap via Homebrew..."

    local attempt=0
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ((attempt++)) || true

        if brew uninstall nmap 2>&1; then
...
verify_uninstall() {
    ...
    if is_nmap_installed_via_brew; then
        log_error "Nmap is still installed via Homebrew"
        return 1
    fi
...
# Main entry point
main() {
    log_info "Starting Nmap uninstallation on macOS..."
    ...
    if ! is_nmap_installed_via_brew; then
        log_info "Nmap is not installed via Homebrew, nothing to uninstall"
        exit 0
    fi
