#!/usr/bin/env bash
# Install script for Nmap on macOS
...
install_nmap() {
    log_info "Installing Nmap via Homebrew..."

    if brew list nmap &> /dev/null; then
        log_info "Nmap is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade nmap 2>&1 || true; then
                log_success "Nmap upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting Nmap installation on macOS..."
