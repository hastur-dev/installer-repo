#!/usr/bin/env bash
# Install script for SQLite on macOS
# Uses Homebrew as the package manager
# Note: SQLite comes pre-installed on macOS, but Homebrew provides a newer version
...
install_sqlite() {
    log_info "Installing SQLite via Homebrew..."

    # Note: macOS includes sqlite3, but Homebrew provides a more up-to-date version
    if brew list sqlite &> /dev/null; then
        log_info "SQLite is already installed via Homebrew, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade sqlite 2>&1 || true; then
                log_success "SQLite upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        if brew install sqlite 2>&1; then
...
# Main entry point
main() {
    log_info "Starting SQLite installation on macOS..."
