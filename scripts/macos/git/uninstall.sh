#!/usr/bin/env bash
# Uninstall script for Git on macOS
# Removes Git installed via Homebrew
...
uninstall_git() {
    log_info "Uninstalling Git via Homebrew..."

    local attempt=0
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ((attempt++)) || true

        if brew uninstall git 2>&1; then
...
verify_uninstall() {
    ...
    if is_git_installed_via_brew; then
        log_error "Git is still installed via Homebrew"
        return 1
    fi

    log_success "Homebrew Git has been removed"
    log_info "Note: Xcode Command Line Tools git may still be available at /usr/bin/git"
    return 0
}

# Main entry point
main() {
    log_info "Starting Git uninstallation on macOS..."
    ...
    if ! is_git_installed_via_brew; then
        log_info "Git is not installed via Homebrew, nothing to uninstall"
        exit 0
    fi
