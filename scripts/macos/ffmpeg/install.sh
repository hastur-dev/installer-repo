...
install_ffmpeg() {
    ...
    if brew list ffmpeg &> /dev/null; then
        log_info "FFmpeg is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade ffmpeg 2>&1 || true; then
                log_success "FFmpeg upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting FFmpeg installation on macOS..."
