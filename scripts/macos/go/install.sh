#!/usr/bin/env bash
# Install script for Go on macOS
...
install_go() {
    ...
    if brew list go &> /dev/null; then
        log_info "Go is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade go 2>&1 || true; then
...
# Main entry point
main() {
    log_info "Starting Go installation on macOS..."
