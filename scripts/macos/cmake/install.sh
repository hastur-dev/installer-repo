#!/usr/bin/env bash
# Install script for CMake on macOS
...
install_cmake() {
    ...
    if brew list cmake &> /dev/null; then
        log_info "CMake is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade cmake 2>&1 || true; then
                log_success "CMake upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        if brew install cmake 2>&1; then
...
# Main entry point
main() {
    log_info "Starting CMake installation on macOS..."
