#!/usr/bin/env bash
# Install script for AWS CLI on macOS
...
install_awscli() {
    ...
    if brew list awscli &> /dev/null; then
        log_info "AWS CLI is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade awscli 2>&1 || true; then
                log_success "AWS CLI upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting AWS CLI installation on macOS..."
