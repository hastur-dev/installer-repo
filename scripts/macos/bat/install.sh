#!/usr/bin/env bash
# Install script for bat on macOS
...
install_bat() {
    ...
    if brew list bat &> /dev/null; then
        log_info "bat is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade bat 2>&1 || true; then
                log_success "bat upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
verify_installation() {
    ...
    if ! command -v bat &> /dev/null; then
        log_error "bat command not found after installation"
        return 1
    fi
...
# Main entry point
main() {
    log_info "Starting bat installation on macOS..."
