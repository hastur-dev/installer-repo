#!/usr/bin/env bash
# Install script for ranger on macOS
...
install_ranger() {
    log_info "Installing ranger via Homebrew..."

    if brew list ranger &> /dev/null; then
        log_info "ranger is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade ranger 2>&1 || true; then
                log_success "ranger upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        if brew install ranger 2>&1; then
...
# Main entry point
main() {
    log_info "Starting ranger installation on macOS..."
