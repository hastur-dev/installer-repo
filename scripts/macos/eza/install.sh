#!/usr/bin/env bash
# Install script for eza on macOS
...
install_eza() {
    ...
    if brew list eza &> /dev/null; then
        log_info "eza is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade eza 2>&1 || true; then
                log_success "eza upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting eza installation on macOS..."
