#!/usr/bin/env bash
# Install script for tmux on macOS
...
install_tmux() {
    ...
    if brew list tmux &> /dev/null; then
        log_info "tmux is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade tmux 2>&1 || true; then
                log_success "tmux upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting tmux installation on macOS..."
