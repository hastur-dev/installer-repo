#!/usr/bin/env bash
# Install script for nano on macOS
...
install_nano() {
    ...
    if brew list nano &> /dev/null; then
        log_info "nano is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade nano 2>&1 || true; then
                log_success "nano upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting nano installation on macOS..."
