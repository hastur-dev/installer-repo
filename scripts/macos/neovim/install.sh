#!/usr/bin/env bash
# Install script for Neovim on macOS
...
install_neovim() {
    ...
    if brew list neovim &> /dev/null; then
        log_info "Neovim is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade neovim 2>&1 || true; then
                log_success "Neovim upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting Neovim installation on macOS..."
