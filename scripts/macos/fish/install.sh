#!/usr/bin/env bash
# Install script for fish shell on macOS
...
# Install fish with retry
install_fish() {
    log_info "Installing fish via Homebrew..."

    if brew list fish &> /dev/null; then
        log_info "fish is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade fish 2>&1 || true; then
                log_success "fish upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
    ...
    log_error "Failed to install fish after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying fish installation..."

    if ! command -v fish &> /dev/null; then
        log_error "fish command not found after installation"
        return 1
    fi
    ...
    if [ -z "$fish_version" ]; then
        log_error "Could not retrieve fish version"
        return 1
    fi

    log_success "fish verified: ${fish_version}"
    log_info "To set fish as your default shell:"
    log_info "  1. Add fish to /etc/shells: sudo sh -c 'echo $(which fish) >> /etc/shells'"
    log_info "  2. Change your shell: chsh -s $(which fish)"
    return 0
}

# Main entry point
main() {
    log_info "Starting fish installation on macOS..."
