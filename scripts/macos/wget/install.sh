#!/usr/bin/env bash
# Install script for wget on macOS
...
install_wget() {
    log_info "Installing wget via Homebrew..."

    if brew list wget &> /dev/null; then
        log_info "wget is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade wget 2>&1 || true; then
                log_success "wget upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting wget installation on macOS..."
