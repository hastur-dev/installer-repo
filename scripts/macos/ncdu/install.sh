#!/usr/bin/env bash
# Install script for ncdu on macOS
...
install_ncdu() {
    log_info "Installing ncdu via Homebrew..."

    if brew list ncdu &> /dev/null; then
        log_info "ncdu is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade ncdu 2>&1 || true; then
                log_success "ncdu upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting ncdu installation on macOS..."
