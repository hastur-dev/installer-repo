#!/usr/bin/env bash
# Install script for ImageMagick on macOS
...
install_imagemagick() {
    log_info "Installing ImageMagick via Homebrew..."

    if brew list imagemagick &> /dev/null; then
        log_info "ImageMagick is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade imagemagick 2>&1 || true; then
                log_success "ImageMagick upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        if brew install imagemagick 2>&1; then
...
# Main entry point
main() {
    log_info "Starting ImageMagick installation on macOS..."
