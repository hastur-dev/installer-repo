#!/usr/bin/env bash
# Install script for kubectl on macOS
...
install_kubectl() {
    ...
    if brew list kubernetes-cli &> /dev/null; then
        log_info "kubectl is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade kubernetes-cli 2>&1 || true; then
                log_success "kubectl upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        if brew install kubernetes-cli 2>&1; then
...
# Main entry point
main() {
    log_info "Starting kubectl installation on macOS..."
