#!/usr/bin/env bash
# Uninstall script for kubectl on macOS
# Removes kubectl installed via Homebrew
...
# Check if kubectl is installed via Homebrew
is_kubectl_installed_via_brew() {
    if brew list kubernetes-cli &> /dev/null 2>&1; then
        return 0
    fi
    return 1
}
...
uninstall_kubectl() {
    log_info "Uninstalling kubectl via Homebrew..."

    local attempt=0
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ((attempt++)) || true

        if brew uninstall kubernetes-cli 2>&1; then
...
verify_uninstall() {
    ...
    if is_kubectl_installed_via_brew; then
        log_error "kubectl is still installed via Homebrew"
        return 1
    fi
...
# Main entry point
main() {
    log_info "Starting kubectl uninstallation on macOS..."
    ...
    if ! is_kubectl_installed_via_brew; then
        log_info "kubectl is not installed via Homebrew, nothing to uninstall"
        exit 0
    fi
