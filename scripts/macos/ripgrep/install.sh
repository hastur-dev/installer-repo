#!/usr/bin/env bash
# Install script for ripgrep on macOS
...
install_ripgrep() {
    ...
    if brew list ripgrep &> /dev/null; then
        log_info "ripgrep is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade ripgrep 2>&1 || true; then
                log_success "ripgrep upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        if brew install ripgrep 2>&1; then
...
# Main entry point
main() {
    log_info "Starting ripgrep installation on macOS..."
