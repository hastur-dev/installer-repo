#!/usr/bin/env bash
# Install script for curl on macOS
...
install_curl() {
    ...
    if brew list curl &> /dev/null; then
        log_info "curl is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade curl 2>&1 || true; then
                log_success "curl upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting curl installation on macOS..."
