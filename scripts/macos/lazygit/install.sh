#!/usr/bin/env bash
# Install script for lazygit on macOS
...
install_lazygit() {
    ...
    if brew list lazygit &> /dev/null; then
        log_info "lazygit is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade lazygit 2>&1 || true; then
                log_success "lazygit upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting lazygit installation on macOS..."
