#!/usr/bin/env bash
# Install script for jq on macOS
...
install_jq() {
    ...
    if brew list jq &> /dev/null; then
        log_info "jq is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade jq 2>&1 || true; then
                log_success "jq upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting jq installation on macOS..."
