#!/usr/bin/env bash
# Install script for fzf on macOS
...
install_fzf() {
    ...
    if brew list fzf &> /dev/null; then
        log_info "fzf is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade fzf 2>&1 || true; then
                log_success "fzf upgraded successfully"
                return 0
            fi
            sleep $RETRY_DELAY_SECONDS
        done
...
# Main entry point
main() {
    log_info "Starting fzf installation on macOS..."
