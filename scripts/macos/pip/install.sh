#!/usr/bin/env bash
# Install script for pip on macOS
# pip comes with Python, so this ensures Python is installed via Homebrew
...
# Install Python (which includes pip) with retry
install_python() {
    log_info "Installing Python (which includes pip) via Homebrew..."

    if brew list python &> /dev/null || brew list python@3.12 &> /dev/null || brew list python@3.11 &> /dev/null; then
...
# Main entry point
main() {
    log_info "Starting pip installation on macOS..."
