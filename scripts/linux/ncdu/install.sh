#!/usr/bin/env bash
# Install script for ncdu on Linux
...
install_ncdu() {
    ...
    log_info "Installing ncdu using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y ncdu 2>&1; then
                    log_success "ncdu installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y ncdu 2>&1; then
                    log_success "ncdu installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y epel-release 2>&1 && sudo yum install -y ncdu 2>&1; then
                    log_success "ncdu installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm ncdu 2>&1; then
                    log_success "ncdu installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install ncdu after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying ncdu installation..."

    if ! command -v ncdu &> /dev/null; then
        log_error "ncdu command not found after installation"
        return 1
    fi
    ...
    if [ -z "$ncdu_version" ]; then
        log_error "Could not retrieve ncdu version"
        return 1
    fi

    log_success "ncdu verified: ${ncdu_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting ncdu installation on Linux..."
