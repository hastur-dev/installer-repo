#!/usr/bin/env bash
# Install script for Zsh on Linux
...
install_zsh() {
    ...
    log_info "Installing Zsh using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y zsh 2>&1; then
                    log_success "Zsh installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y zsh 2>&1; then
                    log_success "Zsh installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y zsh 2>&1; then
                    log_success "Zsh installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm zsh 2>&1; then
                    log_success "Zsh installed successfully"
                    return 0
                fi
                ;;
...
# Main entry point
main() {
    log_info "Starting Zsh installation on Linux..."
