#!/usr/bin/env bash
# Install script for Pandoc on Linux
...
install_pandoc() {
    ...
    log_info "Installing Pandoc using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y pandoc 2>&1; then
                    log_success "Pandoc installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y pandoc 2>&1; then
                    log_success "Pandoc installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y pandoc 2>&1; then
                    log_success "Pandoc installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm pandoc 2>&1; then
                    log_success "Pandoc installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install Pandoc after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}
...
verify_installation() {
    ...
    if ! command -v pandoc &> /dev/null; then
        log_error "pandoc command not found after installation"
        return 1
    fi
    ...
    if [ -z "$pandoc_version" ]; then
        log_error "Could not retrieve Pandoc version"
        return 1
    fi

    log_success "Pandoc verified: ${pandoc_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting Pandoc installation on Linux..."
    ...
    install_pandoc "$package_manager"
    verify_installation
