#!/usr/bin/env bash
# Uninstall script for fish shell on Linux
...
uninstall_fish() {
    ...
    if is_fish_default_shell; then
        ...
        log_warn "Please change your default shell before uninstalling: chsh -s /bin/bash"
        log_error "Aborting uninstall to prevent login issues"
        return 1
    fi

    log_info "Uninstalling fish using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get remove -y fish 2>&1; then
                    log_success "fish uninstalled successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf remove -y fish 2>&1; then
                    log_success "fish uninstalled successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum remove -y fish 2>&1; then
                    log_success "fish uninstalled successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -R --noconfirm fish 2>&1; then
                    log_success "fish uninstalled successfully"
                    return 0
                fi
                ;;
...
verify_uninstall() {
    ...
    if is_fish_installed; then
        log_error "fish is still installed"
        return 1
    fi

    log_success "fish has been removed"
    return 0
}

# Main entry point
main() {
    log_info "Starting fish uninstallation on Linux..."

    if ! is_fish_installed; then
        log_info "fish is not installed, nothing to uninstall"
        exit 0
    fi
    ...
    uninstall_fish "$package_manager"
    verify_uninstall
