#!/usr/bin/env bash
# Install script for fish shell on Linux
...
install_fish() {
    ...
    log_info "Installing fish using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y fish 2>&1; then
                    log_success "fish installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y fish 2>&1; then
                    log_success "fish installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y fish 2>&1; then
                    log_success "fish installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm fish 2>&1; then
                    log_success "fish installed successfully"
                    return 0
                fi
                ;;
...
verify_installation() {
    ...
    log_info "To set fish as your default shell, run: chsh -s $(which fish)"
    return 0
}

# Main entry point
main() {
    log_info "Starting fish installation on Linux..."
