#!/usr/bin/env bash
# Install script for FFmpeg on Linux
...
install_ffmpeg() {
    ...
    log_info "Installing FFmpeg using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                ...
                ;;
            dnf)
                if sudo dnf install -y ffmpeg 2>&1; then
                    log_success "FFmpeg installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y ffmpeg 2>&1; then
                    log_success "FFmpeg installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm ffmpeg 2>&1; then
                    log_success "FFmpeg installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install FFmpeg after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}
...
verify_installation() {
    ...
    if ! command -v ffmpeg &> /dev/null; then
        log_error "ffmpeg command not found after installation"
        return 1
    fi
    ...
    if [ -z "$ffmpeg_version" ]; then
        log_error "Could not retrieve FFmpeg version"
        return 1
    fi

    log_success "FFmpeg verified: ${ffmpeg_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting FFmpeg installation on Linux..."
