#!/usr/bin/env bash
# Install script for nano on Linux
...
install_nano() {
    ...
    log_info "Installing nano using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y nano 2>&1; then
                    log_success "nano installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y nano 2>&1; then
                    log_success "nano installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y nano 2>&1; then
                    log_success "nano installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm nano 2>&1; then
                    log_success "nano installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install nano after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying nano installation..."

    if ! command -v nano &> /dev/null; then
        log_error "nano command not found after installation"
        return 1
    fi

    local nano_version
    nano_version=$(nano --version 2>&1 | head -n 1)

    if [ -z "$nano_version" ]; then
        log_error "Could not retrieve nano version"
        return 1
    fi

    log_success "nano verified: ${nano_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting nano installation on Linux..."
