#!/usr/bin/env bash
# Uninstall script for SQLite on Linux
...
uninstall_sqlite() {
    ...
    log_info "Uninstalling SQLite using ${package_manager}..."

    log_warn "Note: Some system packages may depend on SQLite."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get remove -y sqlite3 2>&1; then
                    log_success "SQLite uninstalled successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf remove -y sqlite 2>&1; then
                    log_success "SQLite uninstalled successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum remove -y sqlite 2>&1; then
                    log_success "SQLite uninstalled successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -R --noconfirm sqlite 2>&1; then
                    log_success "SQLite uninstalled successfully"
                    return 0
                fi
                ;;
...
verify_uninstall() {
    ...
    if is_sqlite_installed; then
        log_error "SQLite is still installed"
        return 1
    fi

    log_success "SQLite has been removed"
    return 0
}

# Main entry point
main() {
    log_info "Starting SQLite uninstallation on Linux..."

    if ! is_sqlite_installed; then
        log_info "SQLite is not installed, nothing to uninstall"
        exit 0
    fi

    local sqlite_version
    sqlite_version=$(sqlite3 --version 2>&1)
    log_info "Current SQLite installation: ${sqlite_version}"
    ...
    uninstall_sqlite "$package_manager"
    verify_uninstall
