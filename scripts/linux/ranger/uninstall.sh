#!/usr/bin/env bash
# Uninstall script for ranger on Linux
...
uninstall_ranger() {
    ...
    log_info "Uninstalling ranger using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get remove -y ranger 2>&1; then
                    log_success "ranger uninstalled successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf remove -y ranger 2>&1; then
                    log_success "ranger uninstalled successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum remove -y ranger 2>&1; then
                    log_success "ranger uninstalled successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -R --noconfirm ranger 2>&1; then
                    log_success "ranger uninstalled successfully"
                    return 0
                fi
                ;;
...
verify_uninstall() {
    ...
    if is_ranger_installed; then
        log_error "ranger is still installed"
        return 1
    fi

    log_success "ranger has been removed"
    return 0
}

# Main entry point
main() {
    log_info "Starting ranger uninstallation on Linux..."

    if ! is_ranger_installed; then
        log_info "ranger is not installed, nothing to uninstall"
        exit 0
    fi
    ...
    uninstall_ranger "$package_manager"
    verify_uninstall
