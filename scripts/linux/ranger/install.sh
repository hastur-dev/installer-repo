#!/usr/bin/env bash
# Install script for ranger on Linux
...
install_ranger() {
    ...
    log_info "Installing ranger using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y ranger 2>&1; then
                    log_success "ranger installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y ranger 2>&1; then
                    log_success "ranger installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y ranger 2>&1; then
                    log_success "ranger installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm ranger 2>&1; then
                    log_success "ranger installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install ranger after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying ranger installation..."

    if ! command -v ranger &> /dev/null; then
        log_error "ranger command not found after installation"
        return 1
    fi

    local ranger_version
    ranger_version=$(ranger --version 2>&1 | head -n 1)

    if [ -z "$ranger_version" ]; then
        log_error "Could not retrieve ranger version"
        return 1
    fi

    log_success "ranger verified: ${ranger_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting ranger installation on Linux..."
    ...
    install_ranger "$package_manager"
    verify_installation
