#!/usr/bin/env bash
# Install script for Git on Linux
...
install_git() {
    ...
    log_info "Installing Git using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y git 2>&1; then
                    log_success "Git installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y git 2>&1; then
                    log_success "Git installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y git 2>&1; then
                    log_success "Git installed successfully"
                    return 0
                fi
                ;;
...
# Main entry point
main() {
    log_info "Starting Git installation on Linux..."
