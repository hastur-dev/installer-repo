#!/usr/bin/env bash
# Install script for wget on Linux
...
install_wget() {
    ...
    log_info "Installing wget using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y wget 2>&1; then
                    log_success "wget installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y wget 2>&1; then
                    log_success "wget installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y wget 2>&1; then
                    log_success "wget installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm wget 2>&1; then
                    log_success "wget installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install wget after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying wget installation..."

    if ! command -v wget &> /dev/null; then
        log_error "wget command not found after installation"
        return 1
    fi
    ...
    if [ -z "$wget_version" ]; then
        log_error "Could not retrieve wget version"
        return 1
    fi

    log_success "wget verified: ${wget_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting wget installation on Linux..."
