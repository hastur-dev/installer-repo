#!/usr/bin/env bash
# Install script for jq on Linux
...
install_jq() {
    ...
    log_info "Installing jq using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y jq 2>&1; then
                    log_success "jq installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y jq 2>&1; then
                    log_success "jq installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y jq 2>&1; then
                    log_success "jq installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install jq after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying jq installation..."
    ...
    if [ -z "$jq_version" ]; then
        log_error "Could not retrieve jq version"
        return 1
    fi

    log_success "jq verified: ${jq_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting jq installation on Linux..."
