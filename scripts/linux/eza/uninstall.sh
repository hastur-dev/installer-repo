#!/usr/bin/env bash
# Uninstall script for eza on Linux
...
uninstall_eza() {
    ...
    log_info "Uninstalling eza using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get remove -y eza 2>&1; then
                    log_success "eza uninstalled successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf remove -y eza 2>&1; then
                    log_success "eza uninstalled successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum remove -y eza 2>&1; then
                    log_success "eza uninstalled successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -R --noconfirm eza 2>&1; then
                    log_success "eza uninstalled successfully"
                    return 0
                fi
                ;;
...
verify_uninstall() {
    ...
    if is_eza_installed; then
        log_error "eza is still installed"
        return 1
    fi

    log_success "eza has been removed"
    return 0
}

# Main entry point
main() {
    log_info "Starting eza uninstallation on Linux..."

    if ! is_eza_installed; then
        log_info "eza is not installed, nothing to uninstall"
        exit 0
    fi
