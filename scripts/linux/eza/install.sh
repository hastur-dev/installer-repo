#!/usr/bin/env bash
# Install script for eza on Linux
...
install_eza() {
    ...
    log_info "Installing eza using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                # eza may need to be installed from a PPA or via cargo on older Ubuntu
                if sudo apt-get install -y eza 2>&1; then
                    log_success "eza installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y eza 2>&1; then
                    log_success "eza installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y eza 2>&1; then
                    log_success "eza installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm eza 2>&1; then
                    log_success "eza installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install eza after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying eza installation..."

    if ! command -v eza &> /dev/null; then
        log_error "eza command not found after installation"
        return 1
    fi

    local eza_version
    eza_version=$(eza --version 2>&1 | head -n 1)

    if [ -z "$eza_version" ]; then
        log_error "Could not retrieve eza version"
        return 1
    fi

    log_success "eza verified: ${eza_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting eza installation on Linux..."
