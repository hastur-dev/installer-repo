#!/usr/bin/env bash
# Uninstall script for Go on Linux
...
uninstall_go() {
    ...
    log_info "Uninstalling Go using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get remove -y golang-go 2>&1; then
                    log_success "Go uninstalled successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf remove -y golang 2>&1; then
                    log_success "Go uninstalled successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum remove -y golang 2>&1; then
                    log_success "Go uninstalled successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -R --noconfirm go 2>&1; then
                    log_success "Go uninstalled successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to uninstall Go after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify uninstallation
verify_uninstall() {
    log_info "Verifying Go uninstallation..."

    if is_go_installed; then
        log_warn "Go is still available in PATH"
        return 1
    fi

    log_success "Go has been removed"
    return 0
}

# Main entry point
main() {
    log_info "Starting Go uninstallation on Linux..."

    if ! is_go_installed; then
        log_info "Go is not installed, nothing to uninstall"
        exit 0
    fi

    local go_version
    go_version=$(go version 2>&1)
    log_info "Current Go installation: ${go_version}"
