#!/usr/bin/env bash
# Install script for tmux on Linux
...
install_tmux() {
    ...
    log_info "Installing tmux using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y tmux 2>&1; then
            ...
            dnf)
                ...
                ;;
            yum)
                if sudo yum install -y tmux 2>&1; then
                    log_success "tmux installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm tmux 2>&1; then
                    log_success "tmux installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install tmux after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}
...
verify_installation() {
    ...
    if ! command -v tmux &> /dev/null; then
        log_error "tmux command not found after installation"
        return 1
    fi
    ...
    if [ -z "$tmux_version" ]; then
        log_error "Could not retrieve tmux version"
        return 1
    fi

    log_success "tmux verified: ${tmux_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting tmux installation on Linux..."
