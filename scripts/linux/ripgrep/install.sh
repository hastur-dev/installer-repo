#!/usr/bin/env bash
# Install script for ripgrep on Linux
...
install_ripgrep() {
    ...
    log_info "Installing ripgrep using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y ripgrep 2>&1; then
                    log_success "ripgrep installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y ripgrep 2>&1; then
                    log_success "ripgrep installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y ripgrep 2>&1; then
                    log_success "ripgrep installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm ripgrep 2>&1; then
                    log_success "ripgrep installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install ripgrep after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}
...
verify_installation() {
    ...
    if ! command -v rg &> /dev/null; then
        log_error "rg command not found after installation"
        return 1
    fi

    local rg_version
    rg_version=$(rg --version 2>&1 | head -n 1)

    if [ -z "$rg_version" ]; then
        log_error "Could not retrieve ripgrep version"
        return 1
    fi

    log_success "ripgrep verified: ${rg_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting ripgrep installation on Linux..."
    ...
    install_ripgrep "$package_manager"
    verify_installation
