#!/usr/bin/env bash
# Install script for ImageMagick on Linux
...
# Install ImageMagick with retry
install_imagemagick() {
    local package_manager="$1"
    ...
    log_info "Installing ImageMagick using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y imagemagick 2>&1; then
                    log_success "ImageMagick installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y ImageMagick 2>&1; then
                    log_success "ImageMagick installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y ImageMagick 2>&1; then
                    log_success "ImageMagick installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm imagemagick 2>&1; then
                    log_success "ImageMagick installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install ImageMagick after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying ImageMagick installation..."

    if ! command -v convert &> /dev/null; then
        log_error "convert command not found after installation"
        return 1
    fi

    local imagemagick_version
    imagemagick_version=$(convert --version 2>&1 | head -n 1)

    if [ -z "$imagemagick_version" ]; then
        log_error "Could not retrieve ImageMagick version"
        return 1
    fi

    log_success "ImageMagick verified: ${imagemagick_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting ImageMagick installation on Linux..."
    ...
    install_imagemagick "$package_manager"
    verify_installation
