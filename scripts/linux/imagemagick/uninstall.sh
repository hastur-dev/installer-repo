#!/usr/bin/env bash
# Uninstall script for ImageMagick on Linux
...
uninstall_imagemagick() {
    ...
    log_info "Uninstalling ImageMagick using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get remove -y imagemagick 2>&1; then
                    log_success "ImageMagick uninstalled successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf remove -y ImageMagick 2>&1; then
                    log_success "ImageMagick uninstalled successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum remove -y ImageMagick 2>&1; then
                    log_success "ImageMagick uninstalled successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -R --noconfirm imagemagick 2>&1; then
                    log_success "ImageMagick uninstalled successfully"
                    return 0
                fi
                ;;
...
verify_uninstall() {
    ...
    if is_imagemagick_installed; then
        log_warn "ImageMagick is still available in PATH"
        return 1
    fi

    log_success "ImageMagick has been removed"
    return 0
}

# Main entry point
main() {
    log_info "Starting ImageMagick uninstallation on Linux..."

    if ! is_imagemagick_installed; then
        log_info "ImageMagick is not installed, nothing to uninstall"
        exit 0
    fi

    local imagemagick_version
    imagemagick_version=$(convert --version 2>&1 | head -n 1)
    log_info "Current ImageMagick installation: ${imagemagick_version}"
    ...
    uninstall_imagemagick "$package_manager"
    verify_uninstall
