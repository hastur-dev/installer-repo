#!/usr/bin/env bash
# Install script for fzf on Linux
...
install_fzf() {
    ...
    log_info "Installing fzf using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y fzf 2>&1; then
                    log_success "fzf installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y fzf 2>&1; then
                    log_success "fzf installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y fzf 2>&1; then
                    log_success "fzf installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm fzf 2>&1; then
                    log_success "fzf installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install fzf after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying fzf installation..."

    if ! command -v fzf &> /dev/null; then
        log_error "fzf command not found after installation"
        return 1
    fi
    ...
    if [ -z "$fzf_version" ]; then
        log_error "Could not retrieve fzf version"
        return 1
    fi

    log_success "fzf verified: ${fzf_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting fzf installation on Linux..."
