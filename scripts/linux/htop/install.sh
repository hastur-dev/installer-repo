#!/usr/bin/env bash
# Install script for htop on Linux
...
install_htop() {
    ...
    log_info "Installing htop using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y htop 2>&1; then
                    log_success "htop installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y htop 2>&1; then
                    log_success "htop installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y epel-release 2>&1 && sudo yum install -y htop 2>&1; then
                    log_success "htop installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm htop 2>&1; then
                    log_success "htop installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install htop after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying htop installation..."

    if ! command -v htop &> /dev/null; then
        log_error "htop command not found after installation"
        return 1
    fi
    ...
    if [ -z "$htop_version" ]; then
        log_error "Could not retrieve htop version"
        return 1
    fi

    log_success "htop verified: ${htop_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting htop installation on Linux..."
