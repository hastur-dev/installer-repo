#!/usr/bin/env bash
# Install script for CMake on Linux
...
install_cmake() {
    ...
    log_info "Installing CMake using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y cmake 2>&1; then
                    log_success "CMake installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y cmake 2>&1; then
                    log_success "CMake installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y cmake 2>&1; then
                    log_success "CMake installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm cmake 2>&1; then
                    log_success "CMake installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install CMake after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying CMake installation..."

    if ! command -v cmake &> /dev/null; then
        log_error "cmake command not found after installation"
        return 1
    fi
    ...
    if [ -z "$cmake_version" ]; then
        log_error "Could not retrieve CMake version"
        return 1
    fi

    log_success "CMake verified: ${cmake_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting CMake installation on Linux..."
