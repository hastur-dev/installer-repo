#!/usr/bin/env bash
# Install script for tldr on Linux
...
install_tldr() {
    ...
    log_info "Installing tldr using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y tldr 2>&1; then
                    log_success "tldr installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y tldr 2>&1; then
                    log_success "tldr installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y tldr 2>&1; then
                    log_success "tldr installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm tldr 2>&1; then
                    log_success "tldr installed successfully"
                    return 0
                fi
                ;;
...
# Main entry point
main() {
    log_info "Starting tldr installation on Linux..."
