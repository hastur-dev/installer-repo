#!/usr/bin/env bash
# Uninstall script for Redis on Linux
...
stop_service() {
    ...
    if command -v systemctl &> /dev/null; then
        ...
        log_info "Redis service stopped and disabled"
    fi
}
...
uninstall_redis() {
    ...
    log_warn "This will remove Redis and stop any running Redis instances."

    log_info "Uninstalling Redis using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get remove -y redis-server redis-tools 2>&1; then
                ...
                ;;
            dnf)
                if sudo dnf remove -y redis 2>&1; then
                    log_success "Redis uninstalled successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum remove -y redis 2>&1; then
                    log_success "Redis uninstalled successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -R --noconfirm redis 2>&1; then
                    log_success "Redis uninstalled successfully"
                    return 0
                fi
                ;;
...
verify_uninstall() {
    ...
    if is_redis_installed; then
        log_error "Redis is still installed"
        return 1
    fi

    log_success "Redis has been removed from the system"
    log_info "Note: Redis data in /var/lib/redis may still exist"
    return 0
}

# Main entry point
main() {
    log_info "Starting Redis uninstallation on Linux..."

    if ! is_redis_installed; then
        log_info "Redis is not installed, nothing to uninstall"
        exit 0
    fi
    ...
    stop_service
    uninstall_redis "$package_manager"
    verify_uninstall
