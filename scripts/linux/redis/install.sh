#!/usr/bin/env bash
# Install script for Redis on Linux
...
install_redis() {
    ...
    log_info "Installing Redis using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y redis-server 2>&1; then
                    log_success "Redis installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y redis 2>&1; then
                    log_success "Redis installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y epel-release 2>&1 && sudo yum install -y redis 2>&1; then
                    log_success "Redis installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm redis 2>&1; then
                    log_success "Redis installed successfully"
                    return 0
                fi
                ;;
...
configure_service() {
    ...
    if command -v systemctl &> /dev/null; then
        ...
        log_info "Redis service enabled and started"
    else
        log_info "systemd not found, skipping service configuration"
    fi
}
...
# Main entry point
main() {
    log_info "Starting Redis installation on Linux..."
