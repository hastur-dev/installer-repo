#!/usr/bin/env bash
# Install script for bat on Linux
# Supports Debian/Ubuntu (apt), Fedora/RHEL (dnf/yum), and Arch (pacman)
# Note: On Debian/Ubuntu, command is 'batcat' due to name conflict
...
install_bat() {
    ...
    log_info "Installing bat using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y bat 2>&1; then
                    log_success "bat installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y bat 2>&1; then
                    log_success "bat installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y bat 2>&1; then
                    log_success "bat installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm bat 2>&1; then
                    log_success "bat installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install bat after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying bat installation..."

    # On Debian/Ubuntu, command is batcat, on others it's bat
    local bat_cmd=""
    ...
    if [ -z "$bat_cmd" ]; then
        log_error "bat/batcat command not found after installation"
        return 1
    fi

    local bat_version
    bat_version=$($bat_cmd --version 2>&1)

    if [ -z "$bat_version" ]; then
        log_error "Could not retrieve bat version"
        return 1
    fi

    log_success "bat verified: ${bat_version}"
    if [ "$bat_cmd" = "batcat" ]; then
        log_info "Note: On Debian/Ubuntu, use 'batcat' command or create alias: alias bat=batcat"
    fi
    return 0
}
...
# Main entry point
main() {
    log_info "Starting bat installation on Linux..."
