#!/usr/bin/env bash
# Install script for tree on Linux
...
install_tree() {
    ...
    log_info "Installing tree using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y tree 2>&1; then
                    log_success "tree installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y tree 2>&1; then
                    log_success "tree installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y tree 2>&1; then
                    log_success "tree installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm tree 2>&1; then
                    log_success "tree installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install tree after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying tree installation..."

    if ! command -v tree &> /dev/null; then
        log_error "tree command not found after installation"
        return 1
    fi
    ...
    if [ -z "$tree_version" ]; then
        log_error "Could not retrieve tree version"
        return 1
    fi

    log_success "tree verified: ${tree_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting tree installation on Linux..."
