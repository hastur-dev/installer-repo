#!/usr/bin/env bash
# Uninstall script for Neovim on Linux
...
uninstall_neovim() {
    ...
    log_info "Uninstalling Neovim using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get remove -y neovim 2>&1; then
                    log_success "Neovim uninstalled successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf remove -y neovim 2>&1; then
                    log_success "Neovim uninstalled successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum remove -y neovim 2>&1; then
                    log_success "Neovim uninstalled successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -R --noconfirm neovim 2>&1; then
                    log_success "Neovim uninstalled successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to uninstall Neovim after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify uninstallation
verify_uninstall() {
    log_info "Verifying Neovim uninstallation..."

    if is_neovim_installed; then
        log_error "Neovim is still installed"
        return 1
    fi

    log_success "Neovim has been removed"
    return 0
}

# Main entry point
main() {
    log_info "Starting Neovim uninstallation on Linux..."

    if ! is_neovim_installed; then
        log_info "Neovim is not installed, nothing to uninstall"
        exit 0
    fi
    ...
    uninstall_neovim "$package_manager"
    verify_uninstall
