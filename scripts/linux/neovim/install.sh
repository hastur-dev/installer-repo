#!/usr/bin/env bash
# Install script for Neovim on Linux
...
install_neovim() {
    ...
    log_info "Installing Neovim using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y neovim 2>&1; then
                    log_success "Neovim installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y neovim 2>&1; then
                    log_success "Neovim installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y neovim 2>&1; then
                    log_success "Neovim installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm neovim 2>&1; then
                    log_success "Neovim installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install Neovim after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}
...
verify_installation() {
    ...
    if ! command -v nvim &> /dev/null; then
        log_error "nvim command not found after installation"
        return 1
    fi

    local nvim_version
    nvim_version=$(nvim --version 2>&1 | head -n 1)

    if [ -z "$nvim_version" ]; then
        log_error "Could not retrieve Neovim version"
        return 1
    fi

    log_success "Neovim verified: ${nvim_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting Neovim installation on Linux..."
