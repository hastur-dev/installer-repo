#!/usr/bin/env bash
# Install script for nginx on Linux
...
install_nginx() {
    ...
    log_info "Installing nginx using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y nginx 2>&1; then
                    log_success "nginx installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y nginx 2>&1; then
                    log_success "nginx installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y epel-release 2>&1 && sudo yum install -y nginx 2>&1; then
                    log_success "nginx installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm nginx 2>&1; then
                    log_success "nginx installed successfully"
                    return 0
                fi
                ;;
...
# Main entry point
main() {
    log_info "Starting nginx installation on Linux..."
    ...
    log_info "nginx is listening on port 80 by default"
