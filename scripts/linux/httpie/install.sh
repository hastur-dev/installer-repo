#!/usr/bin/env bash
# Install script for HTTPie on Linux
...
install_httpie() {
    ...
    log_info "Installing HTTPie using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y httpie 2>&1; then
                    log_success "HTTPie installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y httpie 2>&1; then
                    log_success "HTTPie installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y httpie 2>&1; then
                    log_success "HTTPie installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm httpie 2>&1; then
                    log_success "HTTPie installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install HTTPie after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying HTTPie installation..."

    if ! command -v http &> /dev/null; then
        log_error "http command not found after installation"
        return 1
    fi
    ...
    if [ -z "$httpie_version" ]; then
        log_error "Could not retrieve HTTPie version"
        return 1
    fi

    log_success "HTTPie verified: ${httpie_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting HTTPie installation on Linux..."
    ...
    install_httpie "$package_manager"
    verify_installation
