#!/usr/bin/env bash
# Install script for fd on Linux
# Supports Debian/Ubuntu (apt), Fedora/RHEL (dnf/yum), and Arch (pacman)
# Note: Package is fd-find on apt, fd on others
...
install_fd() {
    ...
    log_info "Installing fd using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                # On Debian/Ubuntu, package is fd-find
                if sudo apt-get install -y fd-find 2>&1; then
                    log_success "fd installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y fd-find 2>&1; then
                    log_success "fd installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y fd-find 2>&1; then
                    log_success "fd installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm fd 2>&1; then
                    log_success "fd installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install fd after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying fd installation..."

    # On Debian/Ubuntu, command is fdfind, on others it's fd
    local fd_cmd=""
    ...
    if [ -z "$fd_cmd" ]; then
        log_error "fd/fdfind command not found after installation"
        return 1
    fi

    local fd_version
    fd_version=$($fd_cmd --version 2>&1)

    if [ -z "$fd_version" ]; then
        log_error "Could not retrieve fd version"
        return 1
    fi

    log_success "fd verified: ${fd_version}"
    log_info "Note: On Debian/Ubuntu, use 'fdfind' command or create alias: alias fd=fdfind"
    return 0
}

# Main entry point
main() {
    log_info "Starting fd installation on Linux..."
