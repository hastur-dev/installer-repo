#!/usr/bin/env bash
# Install script for Make on Linux
...
install_make() {
    ...
    log_info "Installing Make using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y make build-essential 2>&1; then
                    log_success "Make installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y make 2>&1; then
                    log_success "Make installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y make 2>&1; then
                    log_success "Make installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm make 2>&1; then
                    log_success "Make installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install Make after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}
...
# Main entry point
main() {
    log_info "Starting Make installation on Linux..."
