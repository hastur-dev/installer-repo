#!/usr/bin/env bash
# Install script for Nmap on Linux
...
install_nmap() {
    ...
    log_info "Installing Nmap using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y nmap 2>&1; then
                    log_success "Nmap installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y nmap 2>&1; then
                    log_success "Nmap installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y nmap 2>&1; then
                    log_success "Nmap installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm nmap 2>&1; then
                    log_success "Nmap installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install Nmap after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying Nmap installation..."

    if ! command -v nmap &> /dev/null; then
        log_error "nmap command not found after installation"
        return 1
    fi
    ...
    if [ -z "$nmap_version" ]; then
        log_error "Could not retrieve Nmap version"
        return 1
    fi

    log_success "Nmap verified: ${nmap_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting Nmap installation on Linux..."
