#!/usr/bin/env bash
# Install script for curl on Linux
...
install_curl() {
    ...
    log_info "Installing curl using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get install -y curl 2>&1; then
                    log_success "curl installed successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf install -y curl 2>&1; then
                    log_success "curl installed successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum install -y curl 2>&1; then
                    log_success "curl installed successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -S --noconfirm curl 2>&1; then
                    log_success "curl installed successfully"
                    return 0
                fi
                ;;
    ...
    log_error "Failed to install curl after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying curl installation..."

    if ! command -v curl &> /dev/null; then
        log_error "curl command not found after installation"
        return 1
    fi
    ...
    if [ -z "$curl_version" ]; then
        log_error "Could not retrieve curl version"
        return 1
    fi

    log_success "curl verified: ${curl_version}"
    return 0
}

# Main entry point
main() {
    log_info "Starting curl installation on Linux..."
